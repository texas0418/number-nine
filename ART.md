...
Gemini-generated, and SHIPPING (Simon, 2026-07-30). Not placeholders: the
Gemini terms were checked and permit commercial use, the prompts were audited
clean, and real 1963-style photography is no longer a pre-ship task
...
## Broadcast Five set (Simon's renders, 2026-07-29)

Same pipeline (originals in ~/Downloads, KEEP: eiizen = map, tgp83e =
pagodas, jvfdtf = van; the latter two arrive framed as album pages — crop
the photo region out of the page before the square crop).

- `map-marsh.jpg` — the ordnance sheet under the b5-triangulate work,
  darkened to lamplight (brightness 0.42) so the amber sites/lines glow on
  it. No text on the map by design; the engine supplies all marks.
- `obj-ness.jpg` — the finale payoff plate: pagoda laboratories and the
  abandoned array on the shingle, one amber window. Cropped square from the
  album-page render (watch the page edge at the crop bottom).
- `obj-van.jpg` — the detector van in the lane, loop aerial up, windows
  fogged from inside. Bookends the opening prose.

## Broadcast Six set (Simon's renders, 2026-07-29)

Same pipeline (album-page renders — crop the photo from the page first).
Originals: a7xz08 = the set laid to the wood, eq86l3 = the stairs. KEEP.

- `obj-setdown.jpg` — night two's altar image: the receiver back-to-the-room
  on the bench, hurricane lamp low (the amber), cable coiled like something
  asleep.
- `obj-stairs.jpg` — the last image before THE CHOICE: down the cellar
  stairs through the doorway, amber light flooding up, and the chair itself
  visible at the bottom, drawn out, waiting (the render out-dreamed the
  prompt; amber softened 0.85 in processing).

## Icon / splash

Glowing phosphor "9" — generated by `scripts/gen-icon.py` (needs the Pillow
venv). This IS the icon (Simon, 2026-07-30). Not awaiting a polish pass;
regenerate from the script if another size is ever needed.
